 ...
 * @version 1.0
 */
public final class MyExcelExportOfTemplateUtil extends BaseExportService {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(MyExcelExportOfTemplateUtil.class);
...
//        mergedRegionHelper = new MergedRegionHelper(sheet);
        ...
        while (index <= sheet.getLastRowNum()) {
            //wbt 调整,在easypoi中多个遍历会导致单元格合并缓存的问题导致合并数据错乱,这个位置直接给它重置
            mergedRegionHelper = new MergedRegionHelper(sheet);
